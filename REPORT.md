...
## 8.5 보너스 — Streamlit 대시보드

분석 결과를 웹 대시보드로 서비스화했다(`app.py`). 자산 비중 슬라이더 5개로 비중을 조절하고(합계 100% 자동 정규화), 기간 프리셋(전체 / 2008 금융위기 / 2020 코로나 / 2022 인플레이션)을 선택하면 누적수익 곡선·CAGR/변동성/최대낙폭/Sharpe 지표 카드·vs SPY 롤링 상관계수가 실시간 갱신된다.

**실행**: `streamlit run app.py` → 브라우저에서 http://localhost:8500

**스케린샷 세트** (프리셋 4개 × 비중 2안 = 8장, `figures/dashboard/`):

| 프리셋 | 60/40 (SPY 60 / TLT 40) | 5자산 (50/15/15/15/5) |
|---|---|---|
| 전체 | ![](figures/dashboard/전체_6040.png) | ![](figures/dashboard/전체_5자산.png) |
| 2008 금융위기 | ![](figures/dashboard/2008금융위기_6040.png) | ![](figures/dashboard/2008금융위기_5자산.png) |
| 2020 코로나 | ![](figures/dashboard/2020코로나_6040.png) | ![](figures/dashboard/2020코로나_5자산.png) |
| 2022 인플레이션 | ![](figures/dashboard/2022인플레이션_6040.png) | ![](figures/dashboard/2022인플레이션_5자산.png) |

**필터/기간 변경 시나리오**: (1) 기본 60/40 상태에서 “2022 인플레이션” 프리셋을 선택하면 클래식 60/40의 낙폭 카드가 크게 악화되는 것을 확인할 수 있다. (2) 같은 2022 기간에서 비중을 5자산 조합으로 바꾸면 TLT/SHY/GLD 분산 덕분에 낙폭이 완화된다. (3) “전체” 기간에서는 5자산 조합이 장기 수익에서 우위임에도 2022 국면에서만 60/40 대비 방어력이 드러난다 — 리포트의 “60/40은 조건부 규칙” 결론을 탐색적으로 재확인할 수 있다.
